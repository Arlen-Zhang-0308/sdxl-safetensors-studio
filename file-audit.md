...
| 文件 | 类型 | 变更 |
|---|---|---|
| `static/app.js` | 修改 | 历史缩略图支持点击或键盘打开全屏预览，点击遮罩空白或按 Esc 关闭 |
| `static/styles.css` | 修改 | 增加深色灯箱遮罩、图片适配、缩放光标、键盘焦点和减少动效兼容样式 |
| `static/index.html` | 修改 | 更新 CSS 与 JavaScript 静态资源版本参数，避免旧缓存影响功能生效 |
| `.musa-manifest.json` | 修改 | 更新生成时间并保持文件目录清单同步 |
